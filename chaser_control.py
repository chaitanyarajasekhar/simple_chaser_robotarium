...
def main():
    ...
    print('error = %f' % float(np.linalg.norm(r.get_poses()[:2,:] - positions[0,:,:])) )

    for i in range(goal_states.shape[0]-1):
        print('Step %d' % int(i+2))
        goal_points = goal_states[i+1, :, :]
        goal_velocities = velocities[i+1,:,:]

        move_to_goal(r, N, si_barrier_cert, goal_points, goal_velocities)#, False)

        print('error = %f' %float(np.linalg.norm(r.get_poses()[:2,:] - positions[i+1,:,:])))
